...
class Window(Frame):
    ...
    def clickStartButton(self):
        ...
        if status != "hidden":
            ...
            file_path = f"/Users/{username}/Documents/captcha1.wav"
            rmt_path = f"/Users/{username}/Documents/captcha2.wav"
            print(file_path)
            urllib.request.urlretrieve(src, file_path)

            data,samplerate=soundfile.read(file_path)
            soundfile.write(rmt_path,data,samplerate, subtype='PCM_16')
            r=sr.Recognizer()
            while True:
                try:                
                    with sr.AudioFile(rmt_path) as source:
        ...
        while flag:
            ...
            url = ''.join([substr, 'gviz/tq?tqx=out:csv&sheet=rmt'])
            print(url)
            ...
            if content.iloc[index+1, 8] == None:
                index = 0
            else:
                index = index + 1
            print(index)
